...
class DeepQLearner:
    ...
    def __init__(self, input_width, input_height, num_actions,
        ...
        self._train = theano.function([], [loss, q_vals], updates=updates,
                                      givens=givens)
        self._dnn_output = theano.function([], q_vals,
                                       givens={states: self.states_shared})
        self._train_lr = []
        self._train_lr.append(theano.function([], [loss, q_vals], 
                                      updates=lasagne.updates.sgd(loss, params, 0.0003),
                                      givens=givens))
        self._train_lr.append(theano.function([], [loss, q_vals], 
                                      updates=lasagne.updates.sgd(loss, params, 0.0002),
                                      givens=givens))
        self._train_lr.append(theano.function([], [loss, q_vals], 
                                      updates=lasagne.updates.sgd(loss, params, 0.0001),
                                      givens=givens))
    ...
    def train(self, states, actions, rewards, next_states, terminals):
        ...
        if (self.freeze_interval > 0 and
            ...
            self.reset_q_hat()
        #loss, _ = self._train()
        train_func = self._train_lr[min(2, self.update_counter / 1000000)]
        loss, _ = train_func()
    ...
    def q_vals(self, state):
        ...
        states[0, ...] = state
        self.states_shared.set_value(states)
        return self._dnn_output()[0]

    def network_out(self, state):
        states = np.zeros((self.batch_size, self.num_frames, self.input_height,
                           self.input_width), dtype=theano.config.floatX)
        states[0, ...] = state
        self.states_shared.set_value(states)
        dnn_output = self._dnn_output()
        return (dnn_output[0])
    ...
    def choose_action(self, state, epsilon):
        ...
        q_vals = self.network_out(state)
        print "q_vals[16]: %f"%(q_vals[16])
        self.max_q_value = q_vals.max()
        return np.argmax(q_vals)

    def choose_action_test(self, state):
        q_vals = self.network_out(state)
        self.max_q_value = q_vals.max()
        if self.max_q_value < Q_THRES:
            return 16
        else:
            return np.argmax(q_vals)

    def save_q_network(self, index):
		cPickle.dump(self, open("../output/qnetwork/qn_"+str(index)+".pkl","w"), -1)
    ...
    def reset_q(self):
        all_params = lasagne.layers.helper.get_all_param_values(self.next_l_out)
        lasagne.layers.helper.set_all_param_values(self.l_out, all_params)
